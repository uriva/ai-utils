...
Deno.test(
  ...
  async () => {
    ...
    assertEquals(
      doNothings.length,
      0,
      "Intermediate retried do_nothing should not be emitted to persistent history",
    );
  },
);

Deno.test(
  "runAgent - nudges model when do_nothing is emitted without prior tool calls on unanswered user message",
  async () => {
    const history: HistoryEvent[] = [
      participantUtteranceTurn({
        name: "user",
    ...
    const fakeCallModel = (): Promise<HistoryEvent[]> => {
      turnCount++;
      if (turnCount === 1) {
        // Model returns only thoughts / empty text (do_nothing) on turn 1
        return Promise.resolve([doNothingEvent()]);
      }
      return Promise.resolve([
        ownUtteranceTurn("It is going great, almost done!"),
      ]);
    };
    ...
    assertEquals(
      ownUtterances.length,
      1,
      "Expected agent to send an own_utterance to the user after being nudged on turn 1",
    );
    assertEquals(ownUtterances[0].text, "It is going great, almost done!");
    const doNothings = history.filter((e) => e.type === "do_nothing");
    assertEquals(
      doNothings.length,
      0,
